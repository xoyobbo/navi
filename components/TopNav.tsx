...
import type { Category } from "@/lib/personalized-categories";
...
export default function TopNav() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100">
      ...
      {/* カテゴリナビ行 */}
      <div className="h-10 border-t border-gray-100 overflow-x-auto" style={scrollHide}>
        <nav className="flex items-center gap-1.5 px-4 h-full" style={{ width: "max-content" }}>
          {categories.map((cat) => (
            <button
              key={cat.label}
              onClick={() => router.push(`/search?q=${encodeURIComponent(cat.keyword)}`)}
              style={{
                position: "relative",
                padding: "4px 12px",
                fontSize: "0.8125rem",
                color: "#1a1a1a",
                borderRadius: "20px",
                background: cat.isPersonalized ? "#fef9f0" : "transparent",
                border: cat.isPersonalized ? "1px solid #c8a96e" : "1px solid transparent",
                fontWeight: cat.isPersonalized ? 600 : 400,
                whiteSpace: "nowrap",
                cursor: "pointer",
              }}
            >
